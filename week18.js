...
function makeTwo() {
	localStorage.setItem('string1','7,6,5');
...
function makeThree() {
	localStorage.setItem('longString',"name=John, age=25");
...
function showEleven() {
	arrayEleven=JSON.parse(localStorage.getItem('arrayEleven'));
	let output = '';
...
const showTen = () => {
	...
	if (arrayTenString) {
		arrayTen=JSON.parse(localStorage.getItem('arrayTen'));
...
const showNine = () => {
	...
	if (arrayNineString) {
		arrayNine=JSON.parse(localStorage.getItem('arrayNine'));
...
const showSeven = () => {
	...
	const output=`В ${city} живёт ${population} человек `;
...
const showSix = () => {
	...
	const output=`Привет! Я ${name}.Мне ${age} лет.`;
...
let numbers = JSON.parse(localStorage.getItem('task19'));
...
const removeFromCart = () => {
	...
	if (cartItems.length > 0) {
		cartItems.pop(); // Удаляем последний элемент из массива cartItems
	
		localStorage.setItem('task21', JSON.stringify(cartItems)); // Сохраняем обновленный массив в Local Storage
	
		console.log('Последний товар удален из корзины и обновленный список сохранен в Local Storage.');
		} else {
...
const showCart = () => {
	...
	let cartItems = JSON.parse(localStorage.getItem('task21'));
	...
	if (cartItems && cartItems.length > 0) {
		...
		practicum23.appendChild(listItem);
	});
	} else {
	practicum23.textContent = 'Корзина пуста';
	}
};
...
const updateCartCount = () => {
	...
	practicum24.textContent=`Количество товаров в корзинe: ${cartCount}`;
};
...
window.addEventListener('load', () => {
	// document.cookie = "username=Кот Учёный";
	// document.cookie=username+"="+Кот Учёный+expires+";path=/";
	console.log("Cookie 'username' установлено.");
});
...
const deleteCookie = (name) => {
	...
	// 1. Метод document.cookie используется для удаления cookie. Он принимает строку, которая содержит имя и значение cookie, разделенные знаком равенства (=).
	// 2. Параметр expires устанавливается на дату в прошлом, чтобы удалить cookie.
